...
In implementing EBR we used a third-party crate, \code{lazy\_static}, which
contains a macro for lazy-initialized static variables. We use this for the
epoch counter, which is an \code{AtomicUsize}. The global list of thread
entries, as well as a queue of ``to-be freed'' garbage is also stored globally.
These three elements are stored in a struct called \code{GlobalState}.
Thread local state is supported by Rust using the \code{thread\_local!} macro.
The thread local data is a pointer to the threads list entry, a counter of how
many times the thread has been pinned, and buffered garbage. The data is put
together in the struct \code{LocalState}.

Memory we want to free is hidden behind the \code{Garbage} struct. This struct
abstracts away the logic of calling the right destructor. This is needed since
...
dropping. The way we have solved this is by sending the data to a closure, so
that the closure keeps track of the data used. The closure is never called, but
upon dropping the closure all of the values moved to it are destroyed. A
advanage of this solution is that it is simple to implement. A disadvantace is
that the closure needs to be heap allocated, which increases the overhead of
adding garbage.
\todo{implmeent tags or something, and profile?}

When using the collections backed by EBR, the user must first obtain a
\code{Pin}, which is a proof that the current thread is pinned. All operations on
the collections requires a pin, such that we know that the memory read is not
yet freed. There is no way to obtain the pin directly --- users must call the
\code{pin} function, and supply a closure which is given the pin as an argument.
This design decision is made in order to make it nearly impossible for users to
...
counter. Every $n$ call to \code{pin}, we try to increment the epoch.
If we succeed at incrementing the epoch we also free as much garbage as allowed
from the global garbage list. This scheme is in a sense fair, in that threads
that pin a lot supposedly create a lot of garbage, and these threads will also
have to clean up once in a while. It does however suffer from the fact that a
single thread is set to clean up all the garbage from other threads.
...
gracefully; however there is still the problem of crashing threads. One solution
...
\code{null}. Neither of these two solutions have been implemented, as this is
mainly a theoretical problem.

We note that both the entry list and garbage queue themselves must be garbage
...
\code{ebr::queue::Queue<Bag>}. The chunking is done thread locally, which also
lowers synchronization overhead, since fewer elements are shared between threads.


\subsection{Hazard Pointers}
\note{How did we implement hp? Trade-offs}
