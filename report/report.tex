...
the language is 1.21.  The language is compiled and typed, and includes features
such as type inference, pattern matching, and a minimal runtime, without a
garbage collector. The language has no formal speficication, and language
changes are done through an RFC (``request for comments'') process.

Despite the lack of a garbage collector, Rust programmers does not have to
manually manage memory: Rust differs from most other languages in that it
features \emph{ownership} semantics: Values are either \emph{owned} or
...
it can be safely \emph{dropped} by running its destructor. This is similar to
\code{std::unique\_ptr} from C++. The main benefit of Rust compared to C++ is
that the Rust has a much stricter ruleset, which is enforced by the compiler.
However, these rules can also be bent or broken if needed.
...
surrounded by curly brackets. The scope operator is \code{::}. Almost all
syntactic constructs in Rust are expressions, including blocks and \code{if}
statements. Expressions terminated by a semicolon have the value of \emph{unit}
(\code{()}). The return value of a function is the value of its block, which
again is the final expression in the block. Alternatively we can use
\code{return}, as in C. Similar to C++, Rust programmers prefers
...
\code{\&mut}s to the same data, and it is not allowed to have one \code{\&mut}
in addition to other references. References are guaranteed to be valid, through
Rusts usage of \emph{lifetimes}. A reference is only allowed to live for as long
as the data it points to. This makes it impossible to return a reference to local
stack variable in a function (which is undefined behaviour in C and C++):

\begin{lstlisting}[firstnumber=last]
fn add_five() -> &u32 { let n: u32 = 1; &n } // ERROR
\end{lstlisting}

Rust is statically typed, such that all variables have a type at compile time.
In \code{let} bindings the type can \emph{usually} be omitted, and the compiler
will infer the type.


Listing~\ref{lst:rust-example} shows a full. Rust program, featuring the most
important syntactic aspects of the language. All types are fully annotated,
although they can, in this example, be inferred.

\todo{Find a good example to show: structs, traits, closure, match, if let, if
expr, block expr, imports}
\todo{Or, just point them to the book?}
\begin{figure}[ht]
\begin{lstlisting}[numbers=left,label=lst:rust-example,caption=An example Rust program]
use std::io::prelude::*;
fn read_lines(vec: &mut Vec<String>) {
    let stdin = std::io::stdin();   // We want to read from stdin
    let handle = stdin.lock();      // Get exclusive access
    for line in handle.lines() {    // Iterate over each line
        match line {                // What did we get?
            Ok(li) => vec.push(li), // A line. Add it to the vec.
            Err(_e) => return,      // Error. We are done.
        }
    }// Now `handle` goes out of scope, so the lock is unlocked.
}
fn main() {
    let mut vec = Vec::new();
    read_lines(&mut vec);
...
\subsection{Beyond Syntax\todo{name}}

The Rust standard library contains two important types for error handling:
\code{Option<T>} and \code{Result<T, E>}, which are both enums. \code{Option} is
used for values that \emph{might} be present, and \code{Result} is used for
operations that might fail. For instance, \code{HashMap::get} returns an
\code{Option} since the key might not be in the map; \code{File::open} returns a
\code{Result} since the file might not exist or we might not have permission.
These types forces programmers to handle errors where they might occur. Both
types also have a method, \code{unwrap}, which return the value they contain, or
crash at runtime if the value is absent.
...
and the destructor will free the memory. Since references can never outlive the
value it references, any other pointer to the value is guaranteed by the
compiler to be valid. This way we get a safe abstraction over heap allocation.
Despite being wrapped in a type it is often not required to use any additional
syntax in order to use the object that is heap allocated; \code{Box<T>}
implements \code{Deref<T>}, such that a \code{\&Box<T>} is automatically
converted to a \code{\&T} by the compiler. \code{Box<T>} however, is not
converted to \code{T}, as the two types have different \code{Drop} methods.
...
the mental model. \note{note on how we must think about memory, but there is
very little room to screw things up.}
...
\subsection{Rationality behind the rules of Rust\todo{name}}
...
dropped. A advanage of this solution is that it is simple to implement. A
disadvantace is that the closure needs to be heap allocated, which increases the
overhead of adding garbage.  \todo{implmeent tags or something, and profile?}
