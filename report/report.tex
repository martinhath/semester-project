...
\code{std::sync::atomic::Ordering}}.  The C11 memory model (\note{from where is
this actually?}) defined five memory orderings: Acquire, Release,
Acquire+Release, Sequentially Consistent, Relaxed, and \note{some more? note
about unused thing?}, Relaxed is the weakest, with no ordering constraints,
\todo{rather write this in terms of exact semantics, eg. that previous writes
must be visible} Acquire forbids subsequent operations to be moved before the
...
\note{This is somewhat backwards: we have already looked at HP informally.} The
