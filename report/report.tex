...
\begin{figure}[ht]
  \begin{tikzpicture}[
    ntitle/.style={node distance=1cm, font=\normalsize},
    ]
    \node (global-title) [anchor=west] {\large{Global State}};
    ...
    \node [ntitle, node distance=0.5cm] (pins-title) [right=of epoch-title.east, anchor=west]
    ...
    {%
      ...
      \draw[ptr] ($ (pins-f-a.east) - (0.2,0) $) -- (pins-f-b.west);
      \draw[ptr] ($ (pins-f-b.east) - (0.2,0) $) -- (pins-f-c.west);
    }
    \node [draw=black!30, fit={(pins-title) (pins-f-c)}] (pins-box) {};

    \node [ntitle, node distance=1.2cm] (garbage-title) [below=of epoch-fig.west, anchor=west]
    ...
    {%
      \node [lnode,node distance=.7cm]  (garbage-f-a)  [below=of garbage-title.west,
        ...
        \node [lnode,node distance=0.7cm]  (garbage-f-b)  [right=of garbage-f-a]
          {\code{(10, bag)} \nodepart{second} };
        \node [lnode,node distance=0.7cm]  (garbage-f-c)  [right=of garbage-f-b]
          {\code{(11, bag)} \nodepart{second} };
        \node [lnode,node distance=0.7cm]  (garbage-f-d)  [right=of garbage-f-c]
          {\code{(12, bag)} \nodepart{second} };

      \draw[ptr] ($ (garbage-f-a.east) - (0.25,0) $) -- (garbage-f-b.west);
      \draw[ptr] ($ (garbage-f-b.east) - (0.25,0) $) -- (garbage-f-c.west);
    ...
    \node (local-title) [anchor=west, below=of garbage-f-a, shift={(0.10,0)}]
      {\large{Local State}};

    \node [ntitle] (epoch-title) [below=of local-title.west, anchor=west] {Epoch:
      \code{AtomicUsize}};
    \node [block,node distance=0.7cm]  (epoch-fig)  [below=of epoch-title.west, anchor=west] {12};
    \node [draw=black!30, fit={(epoch-title) (epoch-fig)}]
      (epoch-box) {};
    ...
    \node [anchor=west, draw=black, dotted, fit={(local-title) (pins-box) (garbage-box)}]
      (local-box) {};

  \end{tikzpicture}
  \caption{The global and thread local data for the implemented EBR
  scheme.\label{fig:ebr-impl}}
\end{figure}
