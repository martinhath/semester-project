...
\section{Hazard Pointers\label{sec:hazard-pointers}}
...
pushing the pointer onto a queue. We would then occationally visit the queue and
...
mindset into \emph{how} we want to implement it. \todo{somthing more here?} We
note that due to time limitations the implementation is yet to be properly
optimized. The data structures were first implemented without any regard for
memory reclamation. Allocated memory were simply leaked when no longer needed.
...
since the ndoe they are currently on are being removed. This is especially a
...
that a single thread is set to clean up all the garbage from potentiall all
...
As we saw in Section~\ref{sec:hazard-pointers} we have primarily two ways
...
locally and then pushed into a global list. Occationally we pop the hazard
...
absence of bugs''\cite{asd}. In the concurrent world we should be considered
lucky if our tests even show the presence of the bugs they are intended to
catch.
...
happend earlier. In addition, since Valgrind is effectively a virtual machine,
the scheduling is different from when ran by the OS\@. This often made bugs
diasppear, and we were forced to run the tests, or benchmarks, tens of times
...
\begin{figure}[ht]
  ...
  \foreach \kind in {laptop,gribb,server,ryzen,scaleway} {
    \begin{subfigure}{0.24\textwidth}
      ...
      \includegraphics[width=\textwidth]{plots/\kind-b:queue_pop.pdf}
    \end{subfigure}
    \begin{subfigure}{0.24\textwidth}
      \centering \footnotesize\code{Queue::Transfer}
      \includegraphics[width=\textwidth]{plots/\kind-b:queue_transfer.pdf}
    \end{subfigure}
    \begin{subfigure}{0.24\textwidth}
      \centering \footnotesize\code{List::Remove}
      \includegraphics[width=\textwidth]{plots/\kind-b:list_remove.pdf}
    \end{subfigure}
    \begin{subfigure}{0.24\textwidth}
      \centering \footnotesize\code{List::Real}
      \includegraphics[width=\textwidth]{plots/\kind-b:list_real.pdf}
    \end{subfigure}
    \\
  }
\end{figure}
...
\code{HP}. \todo{run again and comment if it is reproducible.}
