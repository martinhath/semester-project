...
thread A is reading the address of a nodes \code{next} field, and then gets
preempted. In the meantime, thread B removes that \code{next} node, reclaims its
...
continues execution, it observes no change, as the address of the \code{next}
...
\chapter{Memory Reclamation} \note{What is memory reclamation? Why is it hard?
What approaches are there?} \note{Assumed background: had OS, have programmed in
C or C++.}
